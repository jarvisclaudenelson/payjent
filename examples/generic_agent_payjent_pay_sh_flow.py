"""Minimal generic-agent Payjent + pay.sh flow.
...
def main() -> None:
    ...
    with Session(engine) as session:
        create_bot_credential(session, "generic-agent", "local-agent-key", settings.signing_secret)
    ...
    try:
        with TestClient(app) as test_client:
            payjent = PayjentClient("http://testserver", api_key="local-agent-key", client=test_client)
            bridge = AgentPayjentBridge(
                payjent,
                bot_id="generic-agent",
                store=MemoryPendingPremiumRequestStore(),
                public_base_url="http://testserver",
            )

            print("user ask: Agent, fetch premium Lisbon weather data from pay.sh")
