...
def dashboard_agent(agent_id: str, request: Request, session: Session = Depends(get_session), settings: Settings = Depends(get_settings)):
    ...
    return f"<!doctype html><html><head><title>{_html_escape(agent.name)} · Payjent</title>{_DASHBOARD_CSS}</head><body><main><div class='topbar'><a href='/dashboard'>← Dashboard</a><form method='post' action='/auth/logout'><button class='logout' type='submit'>Log out</button></form></div><section class='hero'><span class='pill'>{_html_escape(agent.status)}</span><h1>{_html_escape(agent.name)}</h1><p class='muted'>{_html_escape(agent.platform)} · <code>{_html_escape(agent.bot_id)}</code></p></section><div class='grid'><div class='card'><h3>Current policy defaults</h3>{_policy_defaults_html(x402_caps)}</div><div class='card'><h3>Smoke-test setup</h3><ol class='checklist'><li>Keep the Payjent agent credential in the agent secret store.</li><li>Give the agent <a href='/docs/agent-payjent-self-setup.md'>the setup guide</a>.</li><li>Run the demo smoke test and confirm payment creates a one-time resumable action.</li></ol></div></div>{discovery_card}<div class='grid'>{rail_cards}</div><div class='grid'><div class='card'><h3>Agent Install Link</h3>{install_link_form}</div><div class='card'><h3>Agent credential</h3>{credential_form}</div><div class='card'><h3>Danger zone</h3>{revoke_form}{delete_form}</div><div class='card'><h3>Payment rails</h3><p class='muted'>After install, ask your agent to connect the required payment rail and confirm task-budget readiness. Payjent stores only safe rail summaries here.</p><ol class='checklist'><li>Ask the agent to check Payjent capabilities.</li><li>Have it connect Stripe, Decal, x402, or a managed provider as needed.</li><li>Run a paid-action smoke test from the agent, not from copied code.</li></ol></div></div><div class='card'><h3>Agent-readable setup checklist</h3><ol class='checklist'><li>Give this agent a fresh one-time Agent Install Link.</li><li>Ask the agent to redeem it into its private secret store.</li><li>Ask the agent to fetch Payjent capabilities and run a quote → approval → resume smoke test.</li><li>Do not paste raw credentials, tokens, or environment lines into chat.</li></ol></div><div class='card'><h3>Recent payments / spend ledger</h3><h3>Spend ledger entries</h3><p>{len(quotes)} recent quotes</p><ul>{ledger}</ul></div><div class='card'><h3>Paid-action lifecycle ledger</h3><table><thead><tr><th>Action</th><th>Quote</th><th>Payment</th><th>Grant</th><th>Fulfillment</th><th>Spend</th></tr></thead><tbody>{lifecycle}</tbody></table></div></main></body></html>"
