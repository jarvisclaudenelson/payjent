...
:root{{--bg:#050711;--panel:#0b1020cc;--line:#ffffff1c;--text:#f7fbff;--muted:#9fb0c8;--cyan:#67e8f9;--violet:#a78bfa;--green:#86efac;--amber:#fde68a}}*{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:radial-gradient(circle at 20% 0%,#172554 0,#050711 34%),linear-gradient(180deg,#050711,#080b14 55%,#02040a);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,Segoe UI,sans-serif;overflow-x:hidden}}body:before{{content:"";position:fixed;inset:0;pointer-events:none;background:linear-gradient(115deg,transparent,#67e8f91a,transparent,#a78bfa18,transparent);background-size:300% 300%;animation:sheen 18s linear infinite}}a{{color:inherit}}.nav{{position:sticky;top:0;z-index:10;backdrop-filter:blur(20px);background:#050711b8;border-bottom:1px solid var(--line)}}.navin{{max-width:1180px;margin:auto;padding:14px 20px;display:flex;align-items:center;justify-content:space-between;gap:16px}}.brand{{font-weight:900;letter-spacing:-.04em}}.brand:before{{content:"";display:inline-block;width:12px;height:12px;margin-right:9px;border-radius:50%;background:radial-gradient(circle,#fff,var(--cyan));box-shadow:0 0 30px var(--cyan)}}.navlinks{{display:flex;gap:18px;align-items:center;color:var(--muted);font-size:14px}}.btn{{display:inline-flex;align-items:center;gap:9px;border:1px solid #ffffff24;border-radius:999px;padding:11px 16px;text-decoration:none;font-weight:800;background:#ffffff0d;box-shadow:0 10px 40px #0006}}.btn.primary{{background:linear-gradient(135deg,#fff,#b8f3ff 45%,#c4b5fd);color:#06111f;border:0}}.hero{{max-width:1180px;margin:auto;padding:92px 20px 70px;display:grid;grid-template-columns:1.03fr .97fr;gap:46px;align-items:center}}.eyebrow{{color:var(--cyan);font-weight:900;text-transform:uppercase;letter-spacing:.16em;font-size:12px}}h1{{font-size:clamp(48px,8vw,92px);line-height:.88;letter-spacing:-.075em;margin:16px 0 22px}}h2{{font-size:clamp(34px,5vw,64px);letter-spacing:-.055em;line-height:.95;margin:0 0 16px}}p{{color:var(--muted);font-size:18px;line-height:1.65}}.hero p{{font-size:21px;max-width:720px}}.cta{{display:flex;gap:12px;flex-wrap:wrap;margin-top:30px}}.metrics{{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:36px}}.metric,.card,.packet,.rail{{border:1px solid var(--line);background:linear-gradient(180deg,#ffffff12,#ffffff08);border-radius:24px;box-shadow:0 30px 90px #0008, inset 0 1px #fff2}}.metric{{padding:16px}}.metric b{{display:block;font-size:24px;color:#fff}}.stage{{position:relative;min-height:520px;border:1px solid var(--line);border-radius:34px;background:radial-gradient(circle at 50% 0,#67e8f936,transparent 33%),linear-gradient(180deg,#111827cc,#050711cc);overflow:hidden;box-shadow:0 40px 120px #020617}}.spine{{position:absolute;left:50%;top:30px;bottom:30px;width:2px;background:linear-gradient(var(--cyan),var(--violet),var(--green));filter:drop-shadow(0 0 18px var(--cyan))}}.packet{{position:absolute;width:260px;padding:16px;animation:float 7s ease-in-out infinite}}.packet small{{color:var(--cyan);font-weight:900}}.p1{{left:26px;top:42px}}.p2{{right:28px;top:164px;animation-delay:-2s}}.p3{{left:58px;bottom:52px;animation-delay:-4s}}.capsule{{position:absolute;left:50%;top:48%;transform:translate(-50%,-50%);width:170px;height:170px;border-radius:42px;background:conic-gradient(from 90deg,var(--cyan),var(--violet),var(--green),var(--cyan));padding:2px;animation:pulse 4s ease-in-out infinite}}.capsule>div{{height:100%;display:grid;place-items:center;text-align:center;border-radius:40px;background:#08111f;font-weight:900}}section.story{{max-width:1180px;margin:auto;padding:80px 20px}}.grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}}.card{{padding:24px}}.card h3{{font-size:22px;margin:8px 0}}.loop{{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;counter-reset:s}}.loop .card:before{{counter-increment:s;content:"0" counter(s);color:var(--cyan);font-weight:900}}.proof{{display:grid;grid-template-columns:.8fr 1.2fr;gap:22px;align-items:stretch}}.thread{{border-left:2px solid #67e8f955;padding-left:22px}}.thread div{{margin:0 0 16px;padding:16px;border:1px solid var(--line);border-radius:18px;background:#ffffff0a}}.dash{{border:1px solid var(--line);border-radius:30px;padding:18px;background:linear-gradient(180deg,#0f172a,#050711);box-shadow:0 40px 120px #000a}}.dashbar{{display:flex;gap:8px;margin-bottom:14px}}.dot{{width:10px;height:10px;border-radius:50%;background:#64748b}}.ledger{{display:grid;gap:10px}}.rail{{padding:14px;display:flex;justify-content:space-between;color:#dbeafe}}.final{{text-align:center;padding-bottom:110px}}@keyframes float{{50%{{transform:translateY(-18px) rotate(1deg)}}}}@keyframes pulse{{50%{{filter:brightness(1.35);box-shadow:0 0 80px #67e8f966}}}}@keyframes sheen{{to{{background-position:300% 0}}}}@media(max-width:850px){{.hero,.proof{{grid-template-columns:1fr}}.grid,.loop,.metrics{{grid-template-columns:1fr}}.navlinks a:not(.btn){{display:none}}.stage{{min-height:460px}}.packet{{position:relative;left:auto;right:auto;top:auto;bottom:auto;margin:18px;width:auto}}.spine,.capsule{{display:none}}}}@media(prefers-reduced-motion:reduce){{*,*:before{{animation:none!important;scroll-behavior:auto!important}}}}
</style></head><body><nav class='nav'><div class='navin'><a class='brand' href='/'>Payjent</a><div class='navlinks'><a href='#loop'>Loop</a><a href='#trust'>Trust</a><a href='/dashboard'>Dashboard</a><a class='btn primary' href='{primary}'>Register your agent</a></div></div></nav><main><section class='hero'><div><div class='eyebrow'>Payment checkpoint for autonomous work</div><h1>Payment-gate agent actions. Resume exactly after payment.</h1><p>Payjent is the payment gate for agent actions: a human registers and authorizes an agent, the agent creates a paid action for premium data, APIs, or pay.sh-style work, the user pays through Payjent, and the agent resumes the exact stored request with a receipt trail.</p><div class='cta'><a class='btn primary' href='{primary}'>Register your agent →</a><a class='btn' href='/docs/agent-payjent-self-setup.md'>Teach your agent Payjent</a></div><div class='metrics'><div class='metric'><b>Gate</b>Before spend</div><div class='metric'><b>Resume</b>Exact request</div><div class='metric'><b>Prove</b>Receipt trail</div></div></div><div class='stage' aria-label='Animated request packets flowing into a Payjent payment gate'><div class='spine'></div><div class='packet p1'><small>REQUEST PACKET</small><h3>Premium weather API</h3><p>$1.50 · awaiting human payment</p></div><div class='packet p2'><small>GATED CAPSULE</small><h3>Stored envelope</h3><p>Hash, callback, amount, user, agent.</p></div><div class='packet p3'><small>RECEIPT TRAIL</small><h3>Ready to resume</h3><p>Paid grant unlocks exact execution.</p></div><div class='capsule'><div>PAYJENT<br>GATE</div></div></div></section><section class='story'><div class='eyebrow'>Why every serious agent needs it</div><h2>Agents need a checkpoint before they spend, unlock, or fulfill premium work.</h2><div class='grid'><div class='card'><h3>Without a gate</h3><p>Agents improvise payment links, lose request context, or execute expensive actions before consent.</p></div><div class='card'><h3>With Payjent</h3><p>Every paid action becomes a luminous capsule: priced, stored, payable, resumable, and auditable.</p></div><div class='card'><h3>For providers</h3><p>Premium data and tool providers get clean paid access patterns without trusting agent-side screenshots.</p></div></div></section><section id='loop' class='story'><div class='eyebrow'>The Payjent loop</div><h2>A precise four-step payment circuit.</h2><div class='loop'><div class='card'><h3>Register</h3><p>Human owner signs in with WorkOS AuthKit when configured, registers the agent, and receives agent credentials.</p></div><div class='card'><h3>Create paid action</h3><p>The agent stores the exact request envelope, amount, cost breakdown, and callback metadata.</p></div><div class='card'><h3>User pays</h3><p>Payjent hosts the payment checkpoint and issues signed receipt/grant artifacts after verified payment.</p></div><div class='card'><h3>Resume & fulfill</h3><p>The agent consumes the grant, resumes the exact stored request, then marks fulfillment. Payjent does not execute downstream pay.sh.</p></div></div></section><section id='trust' class='story proof'><div><div class='eyebrow'>Trust mechanics</div><h2>Proof thread, not payment theater.</h2><p>Payjent binds bot identity, external user, request hash, amount, grant, spend ledger entries, and fulfillment state into a visible trail for operators.</p></div><div class='thread'><div><b>Signed grants</b><br><span>Agents cannot resume premium work until payment issues a valid grant.</span></div><div><b>Spend reasoning</b><br><span>When agents authorize downstream spend, the ledger stores the tool, vendor, amount, rail, status, and provided reason.</span></div><div><b>Fail-closed rails</b><br><span>Experimental rails stay unpaid until settlement semantics are explicitly mapped.</span></div></div></section><section class='story'><div class='eyebrow'>Use cases</div><h2>Built for agent owners and paid tool providers.</h2><div class='grid'><div class='card'><h3>Premium data APIs</h3><p>Charge before unlocking high-cost market data, research, geocoding, weather, or enrichment calls.</p></div><div class='card'><h3>Human-approved work</h3><p>Gate pay.sh-style tasks while preserving exactly what the user approved. Payjent stores and resumes; it does not run pay.sh for you.</p></div><div class='card'><h3>Agent spend controls</h3><p>Track how the agent got paid and what it paid out for data or tools from one dashboard.</p></div></div></section><section class='story proof'><div><div class='eyebrow'>Dashboard reveal</div><h2>A command center for paid agent actions.</h2><p>Operators can inspect agents, quotes, payment state, grants, fulfillment, and real spend-ledger reasons where your agents provide them.</p></div><div class='dash'><div class='dashbar'><span class='dot'></span><span class='dot'></span><span class='dot'></span></div><div class='ledger'><div class='rail'><span>agent registered</span><b>connected</b></div><div class='rail'><span>paid action</span><b>awaiting / paid</b></div><div class='rail'><span>receipt trail</span><b>issued</b></div><div class='rail'><span>spend ledger</span><b>reason-backed</b></div></div></div></section><section class='story final'><div class='eyebrow'>Start the circuit</div><h2>Give your agent a payment checkpoint before its next premium action.</h2><div class='cta' style='justify-content:center'><a class='btn primary' href='{primary}'>Register your agent →</a><a class='btn' href='/docs/agent-payjent-self-setup.md'>Teach your agent Payjent</a></div></section></main></body></html>"""
...
def dashboard(request: Request, session: Session = Depends(get_session), settings: Settings = Depends(get_settings)):
    ...
    paid_minor = sum(q.amount_minor for q in quotes if q.status in {"paid", "fulfilled", "executing"})
    spend_minor = sum(s.amount_minor for s in spends if s.status in {"authorized", "captured"})
    ...
    return f"<!doctype html><html><head><title>Payjent dashboard</title>{_DASHBOARD_CSS}</head><body><main><div class='topbar'><span class='muted'>Signed in as <b>{_html_escape(account.email)}</b></span><form method='post' action='/auth/logout'><button class='logout' type='submit'>Log out</button></form></div><section class='hero'><div class='eyebrow'>Payjent dashboard v0</div><h1>Agent payment command center</h1><p class='muted'>Track how agents got paid, what they requested, and what they paid for data or tools. Real quotes and SpendLedgerEntry.reason are shown when present; empty states are explicit. Stripe Connect, x402, and integration snippets remain on each agent detail page.</p><div class='grid'><div class='card'><h3>Agent registration</h3><pre><code>{register_curl}</code></pre></div><div class='card'><h3>Agents</h3><div class='stat'>{len(agents)}</div><p class='muted'>Registered agent identities with operator-issued credentials.</p></div><div class='card'><h3>Paid action volume</h3><div class='stat'>{_html_escape(_format_money(paid_minor, 'USD'))}</div><p class='muted'>From recent paid/fulfilled quotes in the dashboard sample.</p></div><div class='card'><h3>Downstream spend</h3><div class='stat'>{_html_escape(_format_money(spend_minor, 'USD'))}</div><p class='muted'>Authorized or captured spend ledger total.</p></div></div></section><h2>Agents</h2><div class='grid'>{cards}</div><div class='grid'><div class='card'><h3>Recent agent interactions</h3><div class='timeline'>{interactions}</div></div><div class='card'><h3>Spend reasoning trail</h3><div class='timeline'>{spend_events}</div></div></div></main></body></html>"
